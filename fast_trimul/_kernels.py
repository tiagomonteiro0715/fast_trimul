...
def outgoing(a, b, acc_dtype=cutlass.Float32, atom_layout_mnk=(2, 2, 1)):
    ...
    fn = _compiled(a_, b_, c_, ab, ab, acc_dtype, atom_layout_mnk, lambda x: x, key)
    fn(a_, b_, c_, _stream())
...
class KernelLinear(nn.Module):
    ...
    def raw(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._key != key:                       # prebind weight + compile once
            ...
            self._fn = _compiled(x_, self._wcute, y_, ab, ab, cutlass.Float32,
                                 (2, 2, 1), lambda t: t, ("proj", M, N, K, x.dtype))
            self._key = key
...
def _proj_gate(x, lin_p, lin_g):
    ...
    fn = _compiled(x_, w_, y_, ab, ab, cutlass.Float32, (2, 2, 1), lambda t: t, key)
    fn(x_, w_, y_, _stream())
...
class TriangleMultiplicativeUpdateKernelFused(nn.Module):
    ...
    def _forward_kernels(self, z: torch.Tensor) -> torch.Tensor:
        ...
        return gated_residual(z, self.proj_g.raw(z_norm), self.proj_g.bias,
                              self.proj_out.raw(m_norm), self.proj_out.bias)
    ...
    def _torch_forward(self, z, p):
        ...
        return z + g * F.linear(mc, p[14], p[15])
