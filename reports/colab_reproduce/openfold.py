"""Benchmark: OpenFold (AF2) Triangle-Multiplication versus fast_trimul (Colab).

This script loads OpenFold's own outgoing Triangle-Multiplication layer, also wraps
it in two torch.compile variants, and compares all of them against fast_trimul's
fused kernel. It first confirms that fast_trimul produces the same numbers as the
OpenFold layer (after copying OpenFold's weights into it), then times every variant
over a chosen sequence length and prints a table of latency, throughput, and
memory. Run it on a GPU runtime. The recorded results live in results_openfold.md.
...
PyTorch 2.13 / CUDA 13.0.
    ...
    ▶ Open in Google Colab:
      https://colab.research.google.com/drive/1nWm-Q3oSMjoob2nZ3eFRfaQYHMLj4DKw?usp=sharing
"""

!pip install -q uv

!uv pip install -q --system torch psutil nvidia-ml-py pandas matplotlib tabulate ninja

!uv pip install -q --system --no-build-isolation "openfold @ git+https://github.com/aqlaboratory/openfold.git"
...
def install_scipy_stub():
    """Stub out the pieces OpenFold needs at import time but cannot always load.

    OpenFold imports a compiled attention extension (`attn_core_inplace_cuda`) and
    a little bit of scipy when it loads. On Colab the extension often fails to load
    and scipy may be missing. This function first registers a harmless fake of that
    attention extension (its real symbols are unused by Triangle-Multiplication),
    taking care to let Python's special dunder lookups still fail so tools like
    inspect and torch.compile do not choke on it. It then makes sure scipy is
    importable, using the real one if present and a tiny fake one otherwise.
    """
...
def build_openfold_module(package, module_path, class_name):
    """Find OpenFold's Triangle-Multiplication class and build one for our sizes.
    ...
    with "tri" and contains "mul". Once a class is found, it inspects the
    constructor and passes only the dimension arguments that constructor accepts,
    plus a direction/mode argument set to "outgoing" if the constructor takes one.
    Returns the created module and the class object it came from.
    """
    trimul_class = None
    try:
    ...
    if trimul_class is None:
        ...
        for info in pkgutil.walk_packages(root.__path__, root.__name__ + "."):
            ...
            if trimul_class is not None:
                break
        if trimul_class is None:
            raise ImportError(f"no TriMul class in {package}")
    ...
    for key, val in (("direction", "outgoing"), ("mode", "outgoing")):
        if key in params:
            kwargs[key] = val
            break
...
def run_benchmark(model, name, pair_repr, warmup=5, runs=30):
    ...
    return {
        ...
        "Std (ms)": round(statistics.stdev(times), 2),
...
def benchmark_all(modules, use_fast=True):
    ...
    length. When requested, it captures a CUDA graph for the fast module so its
    launch overhead is removed. It then benchmarks each module in turn, calling any
    module whose forward accepts a `mask` with it. Any failure is caught and
    printed so one broken module does not stop the rest. Returns the result rows.
    """
    print(f"\n===== N = {seq_len} =====")
    ...
    if use_fast and "fast_trimul (ours)" in modules:
        modules["fast_trimul (ours)"].graphed(pair_repr, mask)

    rows = []
    for name, module in modules.items():
        needs_mask = "mask" in inspect.signature(module.forward).parameters
        callable_module = partial(module, mask=mask) if needs_mask else module
        try:
            rows.append(run_benchmark(callable_module, name, pair_repr))
        except Exception as error:
...
try:
    openfold_module, openfold_class = build_openfold_module(
        "openfold", "openfold.model.triangular_multiplicative_update",
        "TriangleMultiplicationOutgoing")
    modules_to_test["OpenFold Baseline"] = openfold_module.to(device).eval()
    print(f"OpenFold loaded -> {openfold_class.__module__}.{openfold_class.__name__}")
except Exception as error:
    print(f"Skipping OpenFold: {type(error).__name__}: {error}")
    traceback.print_exc()
...
if "OpenFold Baseline" in modules_to_test:
    _of = modules_to_test["OpenFold Baseline"]
    modules_to_test["OpenFold +compile"] = torch.compile(_of)
    modules_to_test["OpenFold +compile (reduce-overhead)"] = torch.compile(_of, mode="reduce-overhead")
    print("torch.compile baselines added -> default + reduce-overhead")

if "OpenFold Baseline" in modules_to_test:
    ref = modules_to_test["OpenFold Baseline"]
    for p in ref.parameters():
        p.data.normal_(0, 0.02)
    checker = FastTriangleMultiplication(d_z=pair_dim, d_c=hidden_dim,
                                         mode="outgoing", residual=False).cuda().eval()
    checker.load_openfold_state_dict(ref.state_dict())
    with torch.no_grad():
        ...
        for tag, out in (("without (OpenFold) ", without), ("with (fast_trimul)", with_lib)):
...
rows = benchmark_all(modules_to_test, use_fast=True)

df = show_results(rows)

df.to_csv("/content/openfold_trimul_bench_N%d.csv" % seq_len, index=False)
