"""Benchmark: Boltz-1's Triangle-Multiplication versus fast_trimul (Google Colab).

This script loads Boltz-1's own outgoing Triangle-Multiplication layer, also wraps
...
Boltz layer (after copying Boltz's weights into it), then times every variant over
a chosen sequence length and prints a table of latency, throughput, and memory.
Run it on a GPU runtime. The recorded results live in results_boltz.md.
...
PyTorch 2.13 / CUDA 13.0.
    ...
    ▶ Open in Google Colab:
      https://colab.research.google.com/drive/1STz3qDeke8q9hiFi6WfPAIiGKk63Zn6K?usp=sharing
"""

!pip install -q uv
!uv pip install -q --system boltz torch pandas tabulate psutil nvidia-ml-py
...
try:
    boltz_module, boltz_class = build_trimul(
        "boltz", "boltz.model.layers.triangular_mult", "TriangleMultiplicationOutgoing")
    modules_to_test["Boltz Baseline"] = boltz_module.to(device).eval()
    print(f"Boltz loaded -> {boltz_class.__module__}.{boltz_class.__name__}")
except Exception as error:
    print(f"Skipping Boltz: {type(error).__name__}: {error}")
    traceback.print_exc()
...
if "Boltz Baseline" in modules_to_test:
    _b = modules_to_test["Boltz Baseline"]
    modules_to_test["Boltz +compile"] = torch.compile(_b)
    modules_to_test["Boltz +compile (reduce-overhead)"] = torch.compile(_b, mode="reduce-overhead")
    print("torch.compile baselines added -> default + reduce-overhead")

if "Boltz Baseline" in modules_to_test:
    ref = modules_to_test["Boltz Baseline"]
    for p in ref.parameters():
        p.data.normal_(0, 0.02)
    checker = FastTriangleMultiplication(d_z=pair_dim, d_c=hidden_dim,
                                         mode="outgoing", residual=False).cuda().eval()
    checker.load_weights(ref.state_dict(), source="boltz")
    with torch.no_grad():
        ...
        for tag, out in (("without (Boltz)   ", without), ("with (fast_trimul)", with_lib)):
...
df.to_csv("/content/boltz_trimul_bench_N%d.csv" % seq_len, index=False)
